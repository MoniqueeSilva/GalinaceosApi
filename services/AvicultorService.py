...
class AvicultorService:
    ...
    def getById(self, id):
        logger.info(f"Buscando avicultor pelo id: {id}")
        return self.repository.getById(id)

    def create(self, dados):
        logger.info("Criando novo avicultor")
        avicultor = Avicultor(
            nome=dados.get("nome"),
        ...
        return self.repository.save(avicultor)

    def update(self, id, dados):
        logger.info(f"Atualizando avicultor: {id}")
        ...
        if dados.get("nascimento"):
            avicultor.nascimento = datetime.strptime(dados.get("nascimento"), "%Y-%m-%d")
    ...
    def delete(self, id):
        logger.info(f"Removendo avicultor: {id}")
        avicultor = self.repository.getById(id)

        if avicultor is None:
            return False

        self.repository.delete(avicultor)
